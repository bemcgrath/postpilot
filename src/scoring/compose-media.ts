...
import { EMPTY_MEDIA } from "./media-delta"
...
export function detectComposeMedia(
  ...
  const hasLinkInText = URL_IN_TEXT.test(text)
  if (!container) {
    return { ...EMPTY_MEDIA, hasLink: hasLinkInText }
  }
  ...
  const hasLink = hasLinkInText || composerCard(container) != null
