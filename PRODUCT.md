...
| Store | Status |
|-------|--------|
| Chrome Web Store | **v0.9.0 live** (published 2026-09-02) — ships the Breakdown Preview for free users at n≥5 posts, the upgrade-friction survey at quota exhaustion, the 7-day no-card Pro trial tier, and the rewrite-scoring fixes (hook-length cliff, over-broad fabrication strip). **v0.9.1 uploaded to the CWS Developer Dashboard, pending Google's review** (2026-09-03) — ships the Voice Profile onboarding guide (see Post-launch todo above) plus two invisible internal changes (score-history-storage.ts promoted to packages/core, a latent tier-type fix in rewrite-service.ts). Not live yet; review turnaround is Google's clock, not ours. |
...
- [x] **Voice Profile onboarding gap** — real user feedback (2026-09-03): the free-text Voice Profile box on Settings → Voice gave no guidance on what to actually put in it, so users were "filling it in because they have to" rather than understanding what it does. Root cause: the box was originally built by pasting in the author's own real `voice_profile.md` from a different project, and the parser (`voice-profile-parser.ts`) was tuned against that one document's specific phrasing/table shapes — never made legible to anyone else. Shipped: the Settings → Voice tab now explains what each section (Niche, Hook Preferences, Length, Tone) is for and why, is explicit that it's optional (5+ Sample Posts alone is enough for Voice Match), and has a "Load Example" button backed by a new portable, tested module (`packages/core/src/scoring/voice-profile-example.ts` — verified against the real parser in `voice-profile-example.test.ts`, not just eyeballed). Mirrored into `manual.html`'s Voice Match section as the durable public doc. Two stray copy-paste lines ("Leave Governor and Hooks alone...") removed from the Voice Profile and Sample Posts tabs in passing.
- [ ] **Screen-recorded walkthrough** — same feedback thread asked for a short video covering the full setup-to-post loop (Voice Profile → niche → write/paste a post → analyze → understand the score → refine → post), distinct from the extension-install walkthrough already done during the thread contest. Not made yet — needs an actual screen recording, which isn't something this tooling can produce. A shot-by-shot script to record from is at [`WALKTHROUGH_SCRIPT.md`](./WALKTHROUGH_SCRIPT.md).
